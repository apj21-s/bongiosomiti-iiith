...
<table border="0" cellpadding="0" cellspacing="0" style="margin: 0 auto;">
  <tr><td align="center">
    <div style="display: inline-block; position: relative;">
      <div style="position: absolute; top: -2px; left: 10%; right: 10%; height: 8px; border-top: 1.5px solid #d1bfae; border-radius: 50%;"></div>
      <div style="font-family: 'Noto Serif Bengali', 'SolaimanLipi', serif; font-size: 38px; color: #7a1f1f; line-height: 1; letter-spacing: -1px; margin: 10px 0 0; font-weight: normal;">মহালয়া</div>
      <div style="font-family: Arial, sans-serif; font-size: 9px; letter-spacing: 2px; color: #8e806c; text-transform: uppercase; margin-top: 5px;">Bangiya Samiti</div>
      <div style="position: absolute; bottom: -8px; left: 25%; right: 25%; height: 4px; border-bottom: 1px solid #d1bfae; border-radius: 50%;"></div>
    </div>
  </td></tr>
</table>
`;
...
console.log("Saving email structure...");

const emailFile = 'utils/email.ts';
let content = fs.readFileSync(emailFile, 'utf8');

const startMarker = 'const html = `';
const startIndex = content.indexOf(startMarker, 0); // we want the first one (for sendQRPassEmail)

if (startIndex !== -1) {
    const stringStartIndex = startIndex + startMarker.length;
    // Find the ending backtick of the string
    let endIndex = stringStartIndex;
    let found = false;
    while(endIndex < content.length) {
        if (content[endIndex] === '`') {
             // check if it's the end of the HTML block (line 170 approx)
             // We can just look for the first backtick after </html>
    ...
    if (found) {
        ...
        console.log("Successfully updated utils/email.ts");
    } else {
